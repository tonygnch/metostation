<?php
/**
 * @var \App\View\AppView $this
 * @var \App\Model\Entity\SolarRadiation $solarRadiation
 */
?>
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        <li class="heading"><?= __('Actions') ?></li>
        <li><?= $this->Html->link(__('Edit Solar Radiation'), ['action' => 'edit', $solarRadiation->ID]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete Solar Radiation'), ['action' => 'delete', $solarRadiation->ID], ['confirm' => __('Are you sure you want to delete # {0}?', $solarRadiation->ID)]) ?> </li>
        <li><?= $this->Html->link(__('List Solar Radiation'), ['action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Solar Radiation'), ['action' => 'add']) ?> </li>
    </ul>
</nav>
<div class="solarRadiation view large-9 medium-8 columns content">
    <h3><?= h($solarRadiation->ID) ?></h3>
    <table class="vertical-table">
        <tr>
            <th scope="row"><?= __('ID') ?></th>
            <td><?= $this->Number->format($solarRadiation->ID) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Value') ?></th>
            <td><?= $this->Number->format($solarRadiation->Value) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('DateAndTime') ?></th>
            <td><?= h($solarRadiation->DateAndTime) ?></td>
        </tr>
    </table>
</div>
